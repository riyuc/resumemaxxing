  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="font-jetbrains text-[10px] font-bold tracking-widest flex-1 text-[#c86ea8]">IMAGE</span>
        {!block.src && (
          <button
        ...
        {block.src && (
          <button
            ...
            onMouseDown={e => e.stopPropagation()}
            onClick={e => { e.stopPropagation(); onUpdate({ src: undefined }) }}
          >
            <Trash2 size={10} />
          </button>
        )}
      ...
      <div style={{ background: '#080410' }} onMouseDown={e => e.stopPropagation()}>
        {block.src ? (
          <div className="flex flex-col">
            <img src={block.src} alt={block.caption || 'canvas image'} className="w-full object-contain max-h-64" />
            <input
              value={block.caption ?? ''}
          ...
          >
            <ImageIcon size={28} className="text-[#2a1030] group-hover:text-[#5a2060] transition-colors" />
